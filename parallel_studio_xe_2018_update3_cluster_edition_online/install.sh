...
add_root_to_xhost_acl()
{
    if [ "$USER" != "$(whoami)" ] && [ "$(whoami)" = "root" ]; then 
        if [ -z "$(su $USER -c xhost | grep -i 'si:localuser:root')" ]; then
            su $USER -c 'xhost +si:localuser:root' > /dev/null 2>&1
            root_added_to_xhost_acl="yes"
        fi
    else
        if [ -z "$(xhost | grep -i 'si:localuser:root')" ]; then
...
if [ "$(uname)" != "Darwin" ] && [ "yes" != "${skip_gui_install}" ] && [ "$XDG_SESSION_TYPE" = "wayland" ]; then
    add_root_to_xhost_acl
fi
